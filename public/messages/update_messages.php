<?php
	...
	$to = $_POST['to'];
	$last_read = 0;
		$sql = "SELECT * FROM messages WHERE _to = '$from' AND is_read > 0 ORDER BY _time desc"; 
	    ...
	    $row = mysqli_fetch_assoc($result);
	   	...
	   	$sql_exist = "SELECT * FROM messages WHERE _to = '$from' AND id > $last_read ORDER BY _time desc , 1"; 

	    $result_exist = mysqli_query($conn, $sql_exist);
	    $row_exist = mysqli_fetch_assoc($result_exist);
	   	if($row_exist){
	   		$sql2 = "SELECT * FROM messages WHERE _to = '$from' AND is_read=0 ORDER BY _time asc "; 
